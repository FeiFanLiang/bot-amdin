    ...
    <el-card :body-style="{ padding: '0px' }">
      <div style="padding: 14px;">
        ...
        <div class="bottom clearfix">
          <time class="time"
            >上次结算：{{ lastResetTime }} - {{ operateUserName }}</time
          >
          <el-button type="text" class="button" @click="handleReset"
            >清空结算</el-button
          >
        </div>
      </div>
...
export default {
  ...
  created() {
    ...
    getPoolApi().then(res => {
      ...
      this.lastResetTime = dayjs(res?.lastResetTime).format(
        "YYYY-MM-DD HH:mm:ss"
      );
    ...
    handleReset() {
      this.$alert("确定要重置奖金池吗？", "操作确认", {
        ...
        resetPoolApi().then(() => {
          ...
          getPoolApi().then(res => {
            this.updateTime = dayjs(res?.updateTime).format(
              "YYYY-MM-DD HH:mm:ss"
            );
            this.lastResetTime = dayjs(res?.lastResetTime).format(
              "YYYY-MM-DD HH:mm:ss"
